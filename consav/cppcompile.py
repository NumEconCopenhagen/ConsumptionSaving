...
def setup_nlopt(vs_path=None,download=True,folder='',do_print=False):
    ...
    if download:
        ...
        nloptzip = f'{os.getcwd()}/{folder}/nlopt-2.4.2-dll64.zip'
        urllib.request.urlretrieve(url, nloptzip)

    # b. unzip
    if download:
        filename = os.path.abspath(f'{os.getcwd()}/{folder}nlopt-2.4.2-dll64.zip') 
...
def set_default_options(options):
    ...
    Args:
        ...
        options (dict): compiler options with 
            ...
            path to NLopt library (included if exists, default is cppfuncs/nlopt-2.4.2-dll64/libnlopt-0.lib)
...
def compile(filename,options={},do_print=False):      
    ...
    # c. check for nlopt
    if os.path.isfile(options['nlopt_lib']):
        use_nlopt = True
    else:
        use_nlopt = False
